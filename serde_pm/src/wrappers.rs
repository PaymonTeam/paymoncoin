        ...
        where D: Deserializer<'de>
    {
        ...
        fn checked_boxed_value<T: Identifiable>(type_id: u32, value: T) -> error::Result<Boxed<T>> {
            if type_id != value.type_id() {
                bail!(SerializationError::WrongSVUID);
            }
          ...
          A: SeqAccess<'de>,
{
    seq.next_element_seed(seed)?
        .ok_or_else(|| errconv(SerializationError::WrongLen))
}
          ...
          A: MapAccess<'de>,
{
    ...
    if next_key != expected_key {
        bail!(errconv::<A::Error>(SerializationError::InvalidMapKey));
    }
...
fn errconv<E>(kind: SerializationError) -> E
    where E: DeError
{
