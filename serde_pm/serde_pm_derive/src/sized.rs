...
pub(crate) fn impl_pm_sized(mut container: ast::Container) -> proc_macro2::TokenStream {
    add_pm_sized_trait_bound_if_missing(&mut container);
    let (item_impl_generics, item_ty_generics, item_where_clause) =
        container.generics.split_for_impl();

    let item_name = &container.ident;
    let dummy_const = ident!("_IMPL_PM_SIZED_FOR__{}", item_name);
...
fn add_pm_sized_trait_bound_if_missing(container: &mut ast::Container) {
