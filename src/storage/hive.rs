...
//        let acc1 = Address::from_public_key(&acc1_pk);
...

        {
            ...
            tx.value = 8000;
            tx.data_type = TransactionType::Full;
            tx.snapshot = 1;
            tx.solid = true;
            tx.height = 1;
            tx.address = acc1.clone();
            tx.timestamp = 1531147330u64;
            tx.attachment_timestamp = tx.timestamp + 2;
...
//            dbg!(tx.object.nonce);
            ...
            tx.object.signature_pubkey = coordinator_pk.clone();
            debug!("tx({:?})={}", th1, self.put_transaction(&tx));
        }

        {
            ...
            tx.solid = true;
            tx.height = 1;
            tx.address = acc2.clone();
            tx.timestamp = 1531147330u64 + 1;
            tx.attachment_timestamp = tx.timestamp + 3;
            tx.trunk_transaction = th1.clone();
            tx.branch_transaction = th1.clone();
...
//            dbg!(tx.object.nonce);
            ...
            tx.object.signature = tx.calculate_signature(&coordinator_sk, &coordinator_pk);
            tx.object.signature_pubkey = coordinator_pk.clone();
            debug!("tx({:?})={}", th2, self.put_transaction(&tx));
        }

        {
            ...
            tx.solid = true;
            tx.height = 1;
            tx.timestamp = 1531147330u64 + 2;
            tx.attachment_timestamp = tx.timestamp + 4;
            tx.branch_transaction = th1.clone();
            tx.trunk_transaction = th2.clone();
...
//            dbg!(tx.object.nonce);
            ...
            tx.object.signature = tx.calculate_signature(&coordinator_sk, &coordinator_pk);
            tx.object.signature_pubkey = coordinator_pk.clone();
            debug!("tx({:?})={}", mh1, self.put_transaction(&tx));
...
//                attachment_timestamp: 1531147330u64 + 2,
